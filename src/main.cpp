...
PriceHandler price_handler(price_buffer, PRICE_DOWN_TIME, PRICE_DOWN_STEP, PRICE_MIN);
...
void setup() {
  ...
  billboards_handler.update_buffer(price_buffer);
...
void sendData(int coinType){
  ...
  if(deposited == 0){
    ...
    disp.scroll_string(message_buffer, FUNDS_SHOW_TIME);
...
void loop()
{
  ...
  billboards_handler.run(time, &disp, price_buffer);
}
